...
import { canClaimDailyCredits, canReceiveAdReward } from '@1000ravier/shared';
...
export class WalletService {
  ...
  async claimDailyCredits(userId: string): Promise<{
    ...
    try {
      ...
      const creditsAmount = config.getConfig().credits.dailyClaimAmount;
  ...
  async processAdReward(
    ...
    try {
      ...
      const configCredits = config.getConfig().credits;
      ...
      const creditsAmount = Math.min(rewardValue, configCredits.adRewardAmount);
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          adRewardLimit: configCredits.adRewardDailyLimit
        }
      };
    } catch (error) {
