  ...
  async getNextQuestions(
    ...
    try {
      ...
      if (!validation.isValid) {
        ...
        await db.updateGameSession(sessionId, { status: 'EXPIRED' });
        return { success: false, message: validation.reason || 'Session expired' };
      }

      if (!validation.canResume) {
        return { success: false, message: validation.reason || 'Session completed' };
      }
  ...
  async submitAnswer(
    ...
    try {
      ...
      if (sessionActivity) {
        ...
        if (submissionsPerMinute > config.getConfig().antiCheat.maxSubmissionsPerMinute) {
      ...
      const isCompleted = newStats.current_question_index >= session.total_questions;
      if (isCompleted) {
        newStats['status'] = 'COMPLETED';
        newStats['completed_at'] = new Date().toISOString();
      ...
      await db.createAnalyticsEvent({
        ...
        properties: {
          ...
          progress: newStats.answered_questions / newStats.total_questions
        },
        deviceInfo,
        ipAddress
      });
