...
export class WalletService {
  ...
  async claimAdReward(
    ...
    try {
      ...
      const { data: currentProfile } = await db.getClient()
        .from('profiles')
        .select('credits_balance, total_ad_rewards, ads_watched_today')
        .eq('id', userId)
        .single();
      ...
      const { error: balanceError } = await db.getClient()
        .from('profiles')
        .update({
          credits_balance: (currentProfile?.credits_balance || 0) + creditsAmount,
          total_ad_rewards: (currentProfile?.total_ad_rewards || 0) + creditsAmount,
          ads_watched_today: (currentProfile?.ads_watched_today || 0) + 1,
  ...
  async processCreditsPurchase(
    ...
    try {
      ...
      const { error: balanceError } = await db.getClient()
        .from('profiles')
        .update({
          credits_balance: (currentProfile?.credits_balance || 0) + bundle.credits,
          updated_at: new Date().toISOString()
        })
        .eq('id', userId);
