...
export class LeaderboardService {
  ...
  async finalizePeriod(periodId: string): Promise<{
    ...
    try {
      ...
      for (let i = 0; i < winners.length; i++) {
        ...
        const fraudCheck = await this.performFraudCheck(winner.sessionId, winner.userId);
        
        if (fraudCheck.isSuspicious) {
          fraudCases++;
          this.logger.warn(`Fraud detected in winner: ${winner.userId}`, fraudCheck);
        ...
        if (!error) {
          ...
          await this.updateUserLifetimeEarnings(winner.userId, period.mode.payout);
        }
      }
  ...
  private async updateUserLifetimeEarnings(userId: string, payoutAmount: number) {
    try {
      ...
      const { data: userData } = await db.getClient()
        .from('users')
        .select('lifetime_earnings_ngn')
        .eq('id', userId)
        .single();
      ...
      await db.getClient()
        .from('users')
        .update({
          lifetime_earnings_ngn: (userData?.lifetime_earnings_ngn || 0) + earningsNGN,
          updated_at: new Date().toISOString()
        })
        .eq('id', userId);
